...
4. Disable and re-enable Mahnwesen once. Dolibarr creates the new
   `mahnwesen_attempt`, `mahnwesen_pause` and `mahnwesen_fee` tables; existing
   cases, rules and history remain in place.
...
- automatic attempt count never exceeds the configured limit.
