...
trait DunningManagerMethods3
{
    ...
    public function hasPendingNoticeAtLevel($caseId, $level)
    {
        $sql = 'SELECT rowid FROM '.MAIN_DB_PREFIX.'mahnwesen_history';
        $sql .= ' WHERE fk_case = '.((int) $caseId);
        $sql .= " AND action = 'notice_sending' AND result = 'pending'";
        $sql .= ' AND level = '.((int) $level);
        $sql .= $this->db->plimit(1);
        $resql = $this->db->query($sql);
        if (!$resql) {
     ...
     * @param User $user Acting user
     * @return int|false History row id, false when already sent/pending or on error
     */
    public function reserveNoticeAttempt($case, $recipient, $level, $amount, $message, $user, $mode = 'manual')
    {
        ...
        $sqlLock = 'SELECT rowid FROM '.MAIN_DB_PREFIX.'mahnwesen_case WHERE rowid = '.((int) $case['id']).' FOR UPDATE';
        $resLock = $this->db->query($sqlLock);
        if (!$resLock || !$this->db->fetch_object($resLock)) {
        ...
        $sqlCheck .= ' WHERE fk_case = '.((int) $case['id']).' AND level = '.((int) $level);
        $sqlCheck .= " AND ((action = 'notice_sent' AND result = 'success') OR (action = 'notice_sending' AND result = 'pending'))";
        ...
        if ($existing) {
            $this->error = ((string) $existing->result === 'success')
                ? 'A successful notice already exists for this dunning level.'
                : 'A notice send is already pending for this dunning level.';
            $this->db->rollback();
            return false;
        }
        ...
        $sql .= ((int) $case['entity']).', '.((int) $case['id']).', '.((int) $case['invoice_id']).", 'notice_sending', ".((int) $level).', '.((float) $amount).", '".$this->db->escape((string) $mode)."', 'pending', '".$this->db->escape((string) $recipient)."', '".$this->db->escape((string) $message)."', '".$this->db->escape($this->db->idate(dol_now()))."', ".((is_object($user) && isset($user->id)) ? (int) $user->id : 0).')';
        if (!$this->db->query($sql)) {
            $this->error = $this->db->lasterror();
            $this->db->rollback();
            return false;
        }
        $historyId = (int) $this->db->last_insert_id(MAIN_DB_PREFIX.'mahnwesen_history');
        if ($historyId <= 0) {
            $this->error = 'Unable to get send reservation history id';
            $this->db->rollback();
            return false;
        }
    ...
    public function finalizeNoticeAttempt($historyId, $success, $message, $case, $user)
    {
        ...
        $sqlLock = 'SELECT rowid FROM '.MAIN_DB_PREFIX.'mahnwesen_history WHERE rowid = '.((int) $historyId)." AND action = 'notice_sending' AND result = 'pending' FOR UPDATE";
        $resLock = $this->db->query($sqlLock);
        if (!$resLock || !$this->db->fetch_object($resLock)) {
            $this->error = $this->db->lasterror() ?: 'Pending send reservation not found during audit finalization';
            if ($resLock) {
                $this->db->free($resLock);
            }
            $this->db->rollback();
        ...
        $action = $success ? 'notice_sent' : 'notice_failed';
        $result = $success ? 'success' : 'failed';
        $sql = 'UPDATE '.MAIN_DB_PREFIX.'mahnwesen_history SET';
        $sql .= " action = '".$this->db->escape($action)."'";
        $sql .= ", result = '".$this->db->escape($result)."'";
        $sql .= ", message = '".$this->db->escape((string) $message)."'";
        $sql .= ' WHERE rowid = '.((int) $historyId);
        $sql .= " AND action = 'notice_sending' AND result = 'pending'";
        if (!$this->db->query($sql)) {
            $this->error = $this->db->lasterror();
            $this->db->rollback();
