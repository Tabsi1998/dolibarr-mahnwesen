...
print '<div class="div-table-responsive"><table class="tagtable liste centpercent">';
print '<tr class="liste_titre"><th>ID</th><th>'.$langs->trans('Invoice').'</th><th>'.$langs->trans('DunningStage').'</th><th>'.$langs->trans('Type').'</th><th>'.$langs->trans('Date').'</th><th class="right">'.$langs->trans('Amount').'</th><th>'.$langs->trans('Status').'</th><th>'.$langs->trans('Action').'</th></tr>';
foreach ((array) $fees as $fee) {
    $invoice = mw_attempts_get_invoice($db, (int) $fee['fk_facture'], $invoiceCache); if (!$invoice) { continue; }
    if (!mw_attempts_can_view_invoice($db, $user, $invoice, $visibleSocCache)) { continue; }
    print '<tr class="oddeven" data-kind="'.dol_escape_htmltag((string) ($fee['kind'] ?? 'fee')).'"><td>'.((int) $fee['rowid']).'</td><td>'.$invoice->getNomUrl(1).'</td><td>'.$langs->trans($manager->getStageLabelKey((int) $fee['level'])).'</td><td>'.$langs->trans(($fee['kind'] ?? 'fee') === 'interest' ? 'MahnwesenInterest' : 'MahnwesenDunningFee').'</td><td>'.dol_print_date($db->jdate($fee['date_creation']), 'dayhour').'</td><td class="right">'.price((float) $fee['amount'], 0, $langs, 1, -1, -1, $fee['currency_code']).'</td><td>'.$langs->trans('MahnwesenFeeStatus_'.$fee['status'])
        .(!empty($fee['fk_claim_invoice']) ? ' <a href="'.dol_escape_htmltag(DOL_URL_ROOT.'/compta/facture/card.php?facid='.((int) $fee['fk_claim_invoice'])).'">'.$langs->trans('MahnwesenClaimInvoice').'</a>' : '').'</td><td>';
    if ($fee['status'] === 'open') {
