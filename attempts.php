...
foreach ((array) $fees as $fee) {
    ...
    if (!mw_attempts_can_view_invoice($db, $user, $invoice, $visibleSocCache)) { continue; }
    print '<tr class="oddeven" data-kind="'.dol_escape_htmltag((string) ($fee['kind'] ?? 'fee')).'"><td>'.((int) $fee['rowid']).'</td><td>'.$invoice->getNomUrl(1).'</td><td>'.$langs->trans($manager->getStageLabelKey((int) $fee['level'])).'</td><td>'.$langs->trans(($fee['kind'] ?? 'fee') === 'interest' ? 'MahnwesenInterest' : 'MahnwesenDunningFee').'</td><td>'.dol_print_date($db->jdate($fee['date_creation']), 'dayhour').'</td><td class="right">'.price((float) $fee['amount'], 0, $langs, 1, -1, -1, $fee['currency_code']).'</td><td>'.dol_escape_htmltag($fee['status']).'</td><td>';
    if ($fee['status'] === 'open') {
